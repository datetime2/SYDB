...
$.modalMsg = function (content, type, times) {
    if (type != undefined) {
        var icon = "";
        if (type == 'success') {
            icon = "fa-check-circle";
        ...
        top.layer.msg(content, { icon: icon, time: time, shift: 5 });
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.state == "success") {
                    ...
                    if (data.showtimes) {
                        $.modalMsg(data.message, data.state, data.showtimes);
                    }
                    else {
...
$.fn.formSerialize = function (formdate) {
    ...
    var postdata = {};
    element.find('input,textarea').each(function (r) {
        var $this = $(this);
        var id = $this.attr('id');
    ...
    element.find('select').each(function (r) {
        ...
        if ($this.attr("name").indexOf("select2") >= 0) {
            var arry = [];
...
$.fn.dataGrid = function (options) {
    var defaults = {
        ...
        shrinkToFit: false,
        gridview: true,
        isoperate: false, //是否自定义操作
        isonselectrow: true, //是否有行点击事件
        gridComplete:null
    };
    var options = $.extend(defaults, options);
    var $element = $(this);
...
$.FormatDateBoxFull = function (value) {
    if (!value)
        return '';
    var dt = $.parseToDate(value);
